...
const Product = () => {
	...
	const generateTransactionId = async () => {
		...
		if (transID == null) {
			const transactionID = ((Math.random() + 1).toString(36).substring(2)).toUpperCase();
	...
	const productList = products.map(product =>
		<div className={styles.clubCard} key={product.id}>
			...
			<div className={styles.btnWrapper}>
				...
				<button prod-id={product.id} prod-price={product.price} onClick={handleClick} className={styles.join}>Add to Cart</button>
			</div>
		</div>
	);
		...
		<>
			...
			<div className={styles.container} style={{ position: 'relative' }}>
				<div style={{ position: 'absolute' }}>
					{userMessage !== '' ? <h5 style={{ backgroundColor: 'orange', padding: '5px', borderRadius: '5px' }}>{userMessage}</h5> : ''}
				</div>
				<div className={styles.btnWrapper}>
					...
					<button
						onClick={() => navigate("/Cart")}
						type="button"> {count > 0 ? <span>{count + '  '}</span> : ''}{'  '}<span style={{ marginLeft: '10px' }}>Go to Cart</span></button>
				</div>
