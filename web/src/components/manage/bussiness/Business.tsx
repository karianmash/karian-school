...
const Business = () => {
	...
	const productList = products.map(product =>
		<div className={productstyles.clubCard}>
			<img src={imageUrl + product.image} alt="Cart" style={{ height: '70px' }} />
			<h5>{product.name}</h5>
			...
			<div className={productstyles.btnWrapper}>
				...
				<button onClick={deleteProduct} className={productstyles.join} prod-id={product.id}>Delete</button>
			</div>
		</div>
	);
				...
				<div className={styles.cardContainer} style={{ marginBottom: '10px' }}>
					<div className={styles.card}>
						<div className={styles.cardContent}>
							...
							{currentUser.role == 'business' ? <button
								onClick={() => navigate("/Ads")}
								type="submit">Create Ads</button> : ''}
							{/* <button
								onClick={() => navigate("/Ads")}
					...
					<div className={styles.card}>
						<div className={styles.cardContent}>
							...
							<button
								...
								type="submit">Sell Products</button>
							{currentUser.role !== 'business' ? < button
								onClick={() => navigate("/Product")}
								type="submit">Buy Products</button> : ''}
						</div>
					</div>
				</div>
				<div style={{ marginBottom: '10px', padding: '10px 5px 10px 10px', backgroundColor: 'blueviolet', color: 'white', textTransform: 'uppercase' }}>My Products</div>
