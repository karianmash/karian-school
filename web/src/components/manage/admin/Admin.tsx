	...
	const studentList = students.map(student =>
		<div className={styles.clubCard} key={student.id}>
			...
			<div className={styles.btnWrapper}>
				{student.role == 'student' ? <button user-id={student.id} onClick={() => navigate('/user/update/' + student.id)} className={styles.exit}>Edit</button> : ''}
				<button className={styles.exit} user-id={student.id} onClick={deleteUser}>Delete</button>
			</div>
		</div>
	);
		...
		<>
			<div className={styles.banner}>
				<div className={styles.bannerContent}>
					<h1 className={styles.title} style={{ textTransform: 'capitalize' }}>{role}</h1>
				</div>
			</div>
			<div className={styles.container} style={{ position: 'relative' }}>
				...
				<div className={styles.btnWrapper}>
					...
					<button onClick={fetchUsers} user-role={'student'}>Students</button>
					{loggedUser.role == 'admin' ? <button onClick={fetchUsers} user-role={'school'}>School Admins</button> : ''}
					<button onClick={fetchUsers} user-role={'business'}>Business Owners</button>
